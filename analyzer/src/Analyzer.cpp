...
public:
  SemanticVisitor(clang::ASTContext &Context, const AnalyzerOptions &Options)
      : Context(Context), Sources(Context.getSourceManager()), Options(Options),
        Normalizer(Context) {}
  ...
  bool VisitImportDecl(clang::ImportDecl *Declaration) {
    ...
    // well.
    if (!Exported) {
      if (const clang::Module *Current = Context.getCurrentNamedModule()) {
        ...
        for (const clang::Module *Candidate : Reexported) {
          if (Candidate == Imported ||
              Candidate->getFullModuleName() == Imported->getFullModuleName()) {
            Exported = true;
            break;
          }
        }
  ...
  bool VisitNamedDecl(clang::NamedDecl *Declaration) {
    if (Options.Mode != AnalysisMode::Provider || Declaration->isImplicit() ||
        !isMainFileDeclaration(Declaration, Sources) ||
        !isExportedDeclaration(Declaration)) {
      return true;
    }
    if (llvm::isa<clang::NamespaceDecl, clang::UsingShadowDecl,
                  clang::TemplateTypeParmDecl, clang::NonTypeTemplateParmDecl,
  ...
  bool VisitCXXConstructExpr(clang::CXXConstructExpr *Expression) {
    ...
    if (Expression->getConstructor() != nullptr) {
      addUse(Expression->getConstructor()->getParent(), "constructed_type",
             Expression->getExprLoc());
    }
    return true;
  }
  ...
  bool
  VisitConceptSpecializationExpr(clang::ConceptSpecializationExpr *Expression) {
    if (Options.Mode == AnalysisMode::Consumer) {
...
private:
  void addProviderDeclaration(const clang::NamedDecl *Declaration) {
    ...
    if (Canonical == nullptr || !SeenDeclarations.insert(Canonical).second) {
      return;
    }

    llvm::json::Object Record = Normalizer.record(Canonical);
    if (owningModuleName(Canonical).empty() && !Options.LogicalModule.empty()) {
      Record["owning_module"] = Options.LogicalModule;
      if (llvm::json::Object *Normalized = Record.getObject("normalized")) {
  ...
  void addProbe(const std::string &Id, const clang::VarDecl *Declaration) {
    ...
    Probe["deduced_type"] = canonicalTypeName(Declaration->getType(), Context);
    ...
    if (Initializer != nullptr) {
      ...
      if (const clang::NamedDecl *Selected = selectedDeclaration(Initializer)) {
      ...
      if (const clang::NamedDecl *Specialization =
        ...
        Probe["selected_specialization"] = Normalizer.record(Specialization);
      }
...
AnalyzerAction::CreateASTConsumer(clang::CompilerInstance &Compiler,
  ...
  (void)InputFile;
  return std::make_unique<AnalyzerConsumer>(Compiler.getASTContext(), Options);
}
