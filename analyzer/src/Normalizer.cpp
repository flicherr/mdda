...
templateParameters(const clang::NamedDecl *Declaration) {
  ...
  if (const auto *Value = llvm::dyn_cast<clang::VarTemplateDecl>(Declaration)) {
    return Value->getTemplateParameters();
  }
  if (const auto *Value =
...
DeclarationNormalizer::normalize(const clang::NamedDecl *Declaration) const {
  ...
  if (const auto *Function = llvm::dyn_cast<clang::FunctionDecl>(Declaration)) {
    ...
    for (const clang::ParmVarDecl *Parameter : Function->parameters()) {
      ...
      if (Parameter->hasDefaultArg()) {
        Entry["default_argument"] = expressionText(Parameter->getDefaultArg());
      }
    ...
    if (const clang::AssociatedConstraint &Requires =
      ...
      Result["trailing_requires"] = expressionText(Requires.ConstraintExpr);
    }
    ...
    if (BodyIsInterfaceRelevant && Function->doesThisDeclarationHaveABody()) {
  ...
  if (Record != nullptr) {
    if (const clang::CXXRecordDecl *Definition = Record->getDefinition()) {
      ...
      for (const clang::CXXBaseSpecifier &Base : Definition->bases()) {
        ...
        Entry["access"] = static_cast<int64_t>(Base.getAccessSpecifier());
      ...
      for (const clang::FieldDecl *Field : Definition->fields()) {
        ...
        if (Field->hasInClassInitializer()) {
          Entry["initializer"] = expressionText(Field->getInClassInitializer());
        }
  ...
  if (const auto *Specialization =
          llvm::dyn_cast<clang::ClassTemplateSpecializationDecl>(Declaration)) {
